...
const AdminReports = () => {
  ...
  useEffect(() => {
    const fetchHosts = async () => {
      try {
        ...
        setHosts(data.filter(u => u.role === 'host'));
      } catch (err) {
        console.error('Failed to fetch hosts for report');
      }
    };
    fetchHosts();
  }, [user]);
  ...
  const fetchReport = async () => {
    ...
    try {
      const config = { headers: { Authorization: `Bearer ${user.token}` } };
      const { data } = await axios.get(`http://localhost:5000/api/reports/admin?month=${month}&year=${year}&hostId=${hostId}`, config);
      setReport(data);
    } catch (err) {
  ...
  const exportPDF = () => {
    if (!report) return;
    const doc = new jsPDF();
    const hostName = hostId === 'all' ? 'All Hosts' : hosts.find(h => h._id === hostId)?.name || 'Host';
    ...
    doc.setFontSize(16);
    doc.text(`Admin Booking Report - ${month}/${year} (${hostName})`, 14, 45);
    ...
    try {
      ...
      doc.save(`Admin_Report_${hostName.replace(' ', '_')}_${month}_${year}.pdf`);
    }
  };

  return (
    <div className="bg-white p-6 rounded-3xl border border-slate-200 shadow-sm mt-8">
      <h2 className="text-2xl font-bold mb-6">Monthly Booking Report (Admin)</h2>
      <div className="flex flex-wrap gap-4 items-end mb-6">
        ...
        <div>
          ...
          <select value={hostId} onChange={(e) => setHostId(e.target.value)} className="border rounded-full px-4 py-2 bg-slate-50">
